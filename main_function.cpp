...
{	int i=0,j=0;
	...
	for(i=1;i<=key;i++)//依次设置顶点编号 
	  c.vexs[i].number=i;
	//依次输入顶点信息  
	strcpy(c.vexs[1].name,"南苑");
	strcpy(c.vexs[1].introduction,"学生生活的地方，南苑食堂饭菜香"); 
	strcpy(c.vexs[2].name,"沁苑");
	strcpy(c.vexs[2].introduction,"学生生活的地方，有各种小店，经常举办各种活动"); 
	strcpy(c.vexs[3].name,"欣苑");
	strcpy(c.vexs[3].introduction,"学生生活的地方，食堂饭菜欠佳"); 
	strcpy(c.vexs[4].name,"西苑");
	strcpy(c.vexs[4].introduction,"学生生活的地方，这里卖的烤红薯和板栗很好吃"); 
	strcpy(c.vexs[5].name,"东苑");
	strcpy(c.vexs[5].introduction,"学生生活的地方,三峡大学科技学院学生住的地方"); 
	strcpy(c.vexs[6].name,"图书馆");
	strcpy(c.vexs[6].introduction,"学生们借阅书籍的地方，常用于自习，是学习圣地"); 
	strcpy(c.vexs[7].name,"校医院");
	strcpy(c.vexs[7].introduction,"多用于做核酸"); 
	strcpy(c.vexs[8].name,"J楼");
	strcpy(c.vexs[8].introduction,"教学楼，三楼设有机房"); 
	strcpy(c.vexs[9].name,"中心体育场");
	strcpy(c.vexs[9].introduction,"学生们日常锻炼，上体育课的地方，每年一次的校运会在此举办"); 
	strcpy(c.vexs[10].name,"欣苑篮球场 ");
	strcpy(c.vexs[10].introduction,"打篮球的地方，篮球体育课在此进行"); 
...
void menuwork1() {
	...
	do
	{
		...
		case 7:system("cls");system("D:\\桌面\\数据结构项目\\Project5\\manage_user_information");break;
